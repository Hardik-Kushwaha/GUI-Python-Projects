...
numberframe=tk.LabelFrame(win,bd=0,bg='white')
...
def create_labels():
    for j in range(1,10):
        ...
        globals() ["label%s"%j].grid(row=1,column=j,ipadx=24,ipady=5)
...
def click():
    ...
    if remaining_numbers == []:
        ...
        gameoverlabel=tk.Label(outputnumberframe,text='Game Over!',bg='white',fg='red',font=('Cascadia Code',30))
        gameoverlabel.grid(row=0,column=0)

outputnumberframe=tk.Frame(win,bg='white')
...
currentnumberlabel=tk.Label(outputnumberframe,bg='white',text='Current Number:',fg='black',font=('Cascadia code',18))
currentnumberlabel.grid(row=0,column=1)
currentnumberlabeldynamic=tk.Label(outputnumberframe,text='N/A',bg='dodgerblue',fg='white',font=('Cascadia code',18),relief='groove')
